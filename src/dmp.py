...
# Numpy
import numpy as np  # nopep8 - Otherwise the linter complains for E402

try:
    import quaternion  # nopep8 - Otherwise the linter complains for E402
except ImportError:
    quaternion = None
...
def _require_quaternion():
    if quaternion is None:
        raise ImportError(
            "CartDMP requires the numpy-quaternion package. "
            "Install it only if orientation DMPs are needed."
        )
...
class DMP(object):
    ...
    def __encode_dmp(self):
        ...
        self.weights_pos = np.transpose(np.linalg.lstsq(A, ft, rcond=None)[0])
...
class CartDMP(DMP):
    ...
    def __encode_quaternion_dmp(self, orienation_samples, reallast=False):
        ...
        # Do linear regression in the least square sense
        self.weights_rot = np.linalg.lstsq(A, ft, rcond=None)[0].T
...
def fix_quaternion_sign(quat_array):
    """Fix sudden sign changes in a recorded quaternion trajectory."""
    _require_quaternion()
    ...
    # In case the trajectory ends with the sign flipped, add the last index.
